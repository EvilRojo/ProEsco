...
$cos=$_POST['con'];
    ...
    $docentes=mysqli_query($con,"select * from usuario where (Correo_Usu='$cor') and (Contra_Usu='$cos')")or die("problemas en el query".mysqli_error($con));
    $tutor=mysqli_query($con,"select * from tutor where (Correo_Tutor='$cor') and (Contra_Tutor='$cos')")or die("problemas en el query".mysqli_error($con));
        if($doc=mysqli_fetch_array($docentes)){
            header('Location: opcion_doc.html');
        }else{
            if($usu=mysqli_fetch_array($tutor)){
?>
              <html lang="en">
  <head>
  ...
  <body>
    <nav>
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="container">
                    <a href="registro_alu.php"><button class="btn btn-success btn-block">Registrar Alumno</button></a><br><br>
